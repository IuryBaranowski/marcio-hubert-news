...
import { ENTREVISTAS_IMAGES } from "./entrevistasImages";

export default function EntrevistasVitrine() {
    ...
    <section className="py-12 md:py-16 bg-white">
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-4">
          {ENTREVISTAS_IMAGES.map((src, index) => (
            <button
              key={index}
              type="button"
      ...
      {lightboxIndex !== null && (
        ...
        >
          ...
          >
            <img
              src={ENTREVISTAS_IMAGES[lightboxIndex]}
              alt={`Entrevista ${lightboxIndex + 1}`}
              className="max-h-[90vh] w-auto max-w-full object-contain rounded-lg"
            />
          </div>
          {ENTREVISTAS_IMAGES.length > 1 && (
            <>
              <button
                type="button"
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  setLightboxIndex(
                    (lightboxIndex - 1 + ENTREVISTAS_IMAGES.length) %
                      ENTREVISTAS_IMAGES.length
                  );
                }}
